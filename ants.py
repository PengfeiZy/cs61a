    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                if hasattr(self.ant,'container') and self.ant.can_contain(insect):
                    self.ant.contain_ant(insect)
                elif hasattr(insect,'container') and insect.can_contain(self.ant):
            ...
            if self.ant.name != 'Queen':
                if self.ant is insect:
                    if hasattr(self.ant, 'container') and self.ant.container:
                        if hasattr(self.ant.ant, 'queen_exist') and not self.ant.ant.first_queen:
                            self.ant = None
                            insect.place = None
                        else:
                    ...
                    if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                        if hasattr(self.ant.ant, 'queen_exist') and self.ant.ant.first_queen:
                            pass
                        else:
                            self.ant.ant = None
                            insect.place = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        if self.max_range:
            i = 1
            while current_place.bees == [] and i <= self.max_range:
                current_place = current_place.entrance
                i+=1
            if current_place.name != hive.name:
                self.place.bees = current_place.bees
            return random_or_none(self.place.bees)
        
        elif self.min_range:
            i = 1
            while i <= self.min_range and current_place.name != hive.name:
                current_place = current_place.entrance
                i+=1
            #这是为了找出其最小范围，从而跳过最小范围以前的格子
            if current_place.bees == []:
            # 上面一句很重要，因为开始的时候不为[]，如果直接用while，在循环开始的时候就会报错。同时我们需要判断循环是否到达hive
                while not current_place.bees and current_place.name != hive.name:
                    current_place = current_place.entrance
            if current_place.name != hive.name:
                self.place.bees = current_place.bees
            return random_or_none(self.place.bees)
...
class FireAnt(Ant):
    """FireAnt coks any Bee in its Place when it expires."""
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            for bee in self.place.bees:
                ...
                if bee.armor != 0:
                    self.save_bees.append(bee)
            self.place.bees = self.save_bees
...
class LongThrower(ThrowerAnt):
    ...
    # 这一题花了很长时间，主要问题有以下几个：
    # 1. 调用subclass内的method的时候，可以直接用subcalss的name，从而避免了重复函数的相互覆盖
    # 2. 对于范围大于5，我们可以在调用函数之前循环五次更新self.place. 从而避免了在subclass函数内修改的麻烦
    # 3. 为了保证更新place不会对class本身的元素造成干扰，我们可以设定一个copy值，在copy里面修改然后返回给原始的变量
    def nearest_bee(self,hive):
...
class HungryAnt(Ant):
    ...
    def __init__(self,time_to_digest=3):
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
...
# END Problem 13
    ...
    def __init__(self,armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        ### This is important because we need to figure out which one is first queen. But if 
        ### we change class attribution, the instance attribution will also be changed. So we
        ### we need to create a internal attribution which can only be changed with instance.
        self.armor = armor
    ...
    def action(self, colony):
        ...
        if not self.first_queen:
            ...
            while place_exit != None:
                places.append(place_exit)
                place_exit = place_exit.exit
            for place in places:
                # We introduce a judgement variable to jusge if ants have been braved.
                if place.ant != None:
                    if place.ant not in self.braved_ants:
                        if isinstance(place.ant,TankAnt):
                            place.ant.damage*=2
                            self.braved_ants.append(place.ant)
                            if place.ant.ant != None and place.ant.ant not in self.braved_ants:
                                place.ant.ant.damage*=2
                                self.braved_ants.append(place.ant.ant)
                        elif isinstance(place.ant,BodyguardAnt):
                            if place.ant.ant != None and place.ant.ant not in self.braved_ants:
                                place.ant.ant.damage*=2
                                self.braved_ants.append(place.ant.ant)
                        else:
                        ...
                        if isinstance(place.ant,BodyguardAnt) and place.ant.ant not in self.braved_ants and place.ant.ant != None:
                            place.ant.ant.damage*=2
                            self.braved_ants.append(place.ant.ant)
            super().action(colony)
        # END Problem 13
