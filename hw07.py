...
class Fib():
    ...
    def next(self):
        "*** YOUR CODE HERE ***"
        # Basically it's is a recurssion of method. We set basical situation first, then we need to create an 
        # instance attribute to save the present value, so that we can use it in the next call.
        if self.value == 0:
            result = Fib(1)
        else:
...
class VendingMachine:
    ...
    def restock(self, increase):
        self.number = self.number + increase
    ...
    def vend(self):
        if self.number <= 0:
            ...
            if self.balance < self.price:
                ...
                self.number -= 1
                self.balance = 0
                return 'Here is your {} and ${} change.'.format(self.name, difference)
...
class MissManners:
    ...
    def ask(self, message, *args):
        ...
        "*** YOUR CODE HERE ***"
        attrname = message[(len(magic_word)):]    
        if hasattr(self.obj, attrname):
            return getattr(self.obj, attrname)(*args)
        else:
            return 'Thanks for asking, but I know not how to {}.'.format(attrname)
